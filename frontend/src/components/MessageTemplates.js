...
const MessageTemplates = () => {
  ...
  const handleDelete = async (templateId, templateName) => {
    try {
      await axios.delete(`http://localhost:8000/message-templates/${templateId}`);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (editingTemplate) {
        ...
        await axios.put(`http://localhost:8000/message-templates/${editingTemplate.id}`, payload);
        ...
        // Create new template
        const response = await axios.post('http://localhost:8000/message-templates/', payload);
