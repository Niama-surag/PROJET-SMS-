...
const CampaignManagement = () => {
  ...
  const fetchTemplates = async () => {
    try {
      const response = await axios.get('http://localhost:8000/message-templates/');
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (editingCampaign) {
        ...
        await axios.put(`http://localhost:8000/campaigns/${editingCampaign.id_campagne}`, payload);
        ...
        // Create new campaign
        const response = await axios.post('http://localhost:8000/campaigns/', payload);
  ...
  const handlePauseCampaign = async (campaignId, campaignName) => {
    try {
      await axios.put(`http://localhost:8000/campaigns/${campaignId}`, { statut: 'paused' });
  ...
  const handleStopCampaign = async (campaignId, campaignName) => {
    try {
      await axios.put(`http://localhost:8000/campaigns/${campaignId}`, { statut: 'stopped' });
  ...
  const handleResumeCampaign = async (campaignId, campaignName) => {
    try {
      await axios.put(`http://localhost:8000/campaigns/${campaignId}`, { statut: 'active' });
