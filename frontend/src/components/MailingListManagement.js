...
const MailingListManagement = () => {
  ...
  const fetchMailingLists = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:8001/mailing-lists/');
  ...
  const handleDeleteMailingList = async (listId, listName) => {
    try {
      await axios.delete(`http://localhost:8001/mailing-lists/${listId}`);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (editingList) {
        ...
        await axios.put(`http://localhost:8001/mailing-lists/${editingList.id_liste}`, listData);
        ...
        const response = await axios.post('http://localhost:8001/mailing-lists/', listData);
