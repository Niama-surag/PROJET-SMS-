...
const ContactManagement = () => {
  ...
  const handleDeleteContact = async (contactId) => {
    try {
      await axios.delete(`http://localhost:8001/contacts/${contactId}`);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (editingContact) {
        ...
        await axios.put(`http://localhost:8001/contacts/${editingContact.id_contact}`, contactData);
        ...
        // Create new contact
        const response = await axios.post('http://localhost:8001/contacts/', contactData);
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      await axios.post('http://localhost:8001/contacts/import', formData, {
        headers: {
  ...
  const handleSendSms = async (values) => {
    ...
    try {
      ...
      // Send SMS via API
      await axios.post('http://localhost:8001/sms/send', smsData);
